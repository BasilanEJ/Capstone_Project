    ...
    .weekly-calendar .table th {
    background-color: #f1f5ff; /* subtle header */
}
.weekly-calendar .text-muted em {
    font-size: 13px;
}
    ...
    <script>
        ...
        function setChartType(type) {
            ...
            // Re-render with the last dataset if you want to persist; easiest is to just re-click the last filter.
            // If you want to remember the last range clicked, store it:
            if (window._lastRange) loadSalesData(window._lastRange);
        }

        function loadSalesData(type) {
            window._lastRange = type; // remember last clicked range
        ...
        function updateChart(salesData) {
            ...
            chart = new Chart(ctx, {
                type: currentChartType, // 'bar' or 'line'
                data: {
                    labels: salesData.labels,
                    datasets: [{
                        label: 'Sales (₱)',
                        data: salesData.data,
                        // leave colors default per your preference; Chart.js handles basic styling
                        tension: currentChartType === 'line' ? 0.3 : 0, // slight smoothing for line
                        fill: false
                    }]
                },
                options: {
