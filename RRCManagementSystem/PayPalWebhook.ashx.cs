...
{
    /// <summary>
    /// PayPalWebhook.ashx - Handles PayPal webhooks with email confirmation and PDF receipt generation
    /// </summary>
    public class PayPalWebhook : IHttpHandler
    {
        ...
        private static void HandleGetFallback(HttpContext context)
        {
            ...
            string refFromUi = context.Request.QueryString["ref"];

            if (!int.TryParse(bookingIdStr, out int bookingId))
            {
                LogToFile("❌ GET: Invalid booking id");
            ...
            LogToFile($"=== GET REQUEST ===\nBookingID: {bookingId}, Amount: {amount}, ClientID: {clientId}");

            int saleId = GetOrCreateSaleId(bookingId);
            LogToFile($"SaleID: {saleId}");

            string refToken = !string.IsNullOrWhiteSpace(refFromUi)
                ? $"PAYPAL-GET:{refFromUi}"
                : $"PAYPAL-GET:{bookingId}:{Guid.NewGuid():N}";

            if (ExistsDuplicateRef(saleId, refToken))
            {
                LogToFile($"⚠️ Duplicate reference detected: {refToken}");
            ...
            if (clientId <= 0)
            {
                clientId = GetClientIdFromBooking(bookingId);
            }
            ...
            string receiptFileName = GenerateReceiptFileName(bookingId, "PayPal");
            ...
            string receiptPath = GenerateReceiptPDF(
                ...
                amount, "PayPal", refToken,
                receiptFileName, clientName,
                clientNumber, bookingCode
            );
            ...
            LogToFile($"=== GET REQUEST COMPLETED SUCCESSFULLY ===\n");
            context.Response.Write("✅ DB updated, email sent & blockchain logged (GET).");
        }

        // ========================= POST: Real PayPal webhook =========================
        private static void HandlePostWebhook(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    LogToFile($"Event ignored: {eventType}");
                ...
                string captureId = payload["resource"]?["id"]?.ToString();

                if (!int.TryParse(bookingIdStr, out int bookingId))
                {
                    LogToFile($"❌ Invalid booking id in webhook");
                ...
                if (!decimal.TryParse(amountStr, NumberStyles.Number, CultureInfo.InvariantCulture, out decimal amount) || amount <= 0m)
                {
                    LogToFile($"❌ Invalid amount in webhook");
                    context.Response.Write("❌ Invalid amount in webhook.");
                    return;
                }

                LogToFile($"Parsed - BookingID: {bookingId}, Amount: {amount}, CaptureID: {captureId}");
                ...
                string refToken = !string.IsNullOrWhiteSpace(captureId)
                    ...
                    : $"PAYPAL-POST:{bookingId}:{amount:0.00}";
                ...
                string remarks = string.IsNullOrEmpty(payerEmail)
                ...
                context.Response.Write("✅ PayPal webhook processed. Email sent.");
            }
            catch (Exception ex)
            {
        ...
        private static void LogToFile(string message)
        {
            try
            {
                string logPath = HttpContext.Current.Server.MapPath("~/App_Data/PayPalWebhook.log");
