...
ORDER BY b.CreatedAt DESC";
                ...
                if (reader.Read())
                {
                    ...
                    {
                        decimal amountToPay = CalculateNextInstallment(lblPaymentPlan.Text, price, totalPaid);
                        lblPrice.Text = $@"
        ...
        private decimal CalculateNextInstallment(string plan, decimal fullPrice, decimal totalPaid)
        {
            if (plan.ToLower().Contains("contract"))
            {
                if (totalPaid == 0)
...
ORDER BY b.CreatedAt DESC";
                ...
                if (reader.Read())
                {
                    ...
                    decimal totalPaid = GetTotalPaid(bookingId);
                    amount = CalculateNextInstallment(plan, price, totalPaid);
            ...
            var payload = new
            {
                data = new
                {
                    attributes = new
                    {
                        ...
                        redirect = new
                        {
                            success = $"https://448b-136-158-39-124.ngrok-free.app/PaymentConfirmation.aspx?ref={referenceNumber}",
                            failed = $"https://448b-136-158-39-124.ngrok-free.app/Payment.aspx"
